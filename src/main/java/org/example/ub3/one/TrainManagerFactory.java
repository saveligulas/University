...
public class TrainManagerFactory {
    public static TrainManager createTrainManager(int height, int width, int trolleyCount, int hubCount, int productAmount) {
        TrainNetwork network = new TrainNetwork(height, width);
        MyCollection<Trolley> trolleys = new MyCollection<>();
        MyBidirectionalDictionary<Point, TrainManager.TrainHub> pointHubs = new MyBidirectionalDictionary<>();
        Random rand = new Random();

        for (int i = 0; i < trolleyCount; i++) {
            trolleys.add(new Trolley(network, new Point(0, 0), (int) Math.ceil((double) productAmount / 10)));
        }
        ...
        return new TrainManager(network, trolleys, pointHubs);
    }
}
